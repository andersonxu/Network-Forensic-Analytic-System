...
public class Xgb2 {
			...
			try {
				...
				while ((l = bReader.readLine()) != null) {
					...
					if (cat_pred.length > 1) {
						res = Double.parseDouble(Xgb2.res(root, line)) + Double.parseDouble(cat_pred[1]);
						re = Xgb2.join(line) + comma + cat_pred[0] + divide + res;
					} else {
						res = Double.parseDouble(Xgb2.res(root, line)) + (double) params.get("ini");
						re = l + divide + res;
					}
					bw.write(re);
					bw.newLine();
	...
	private final static void grow(String input_path, Element parent, int depth, String[] set,
		...
		if (settings.getScore() <= 0.0 || depth > (int) params.get("max_depth")) {
			...
			new_set[1] = "l";
			new_set[2] = Double.toString(settings.getP());z
