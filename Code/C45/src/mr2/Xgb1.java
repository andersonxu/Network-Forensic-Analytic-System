...
public class Xgb1 {
			...
			try {
				...
				while ((l = bReader.readLine()) != null) {
					...
					if (cat_pred.length > 1) {
						res = Double.parseDouble(Xgb1.res(root, line)) + Double.parseDouble(cat_pred[1]);
						re = Xgb1.join(line) + comma + cat_pred[0] + divide + res;
					} else {
						res = Double.parseDouble(Xgb1.res(root, line)) + (double) params.get("ini");
						re = l + divide + res;
					}
					bw.write(re);
					bw.newLine();
	...
	private final static void grow(String input_path, Element parent, int depth, String[] set,
		...
		while (why) {
			try {
				...
				Xgb1.sum_mr(input_path, depth, set, params);
				// calculate the gain ratio
				Xgb1.att_mr(input_path, depth, params);
				BufferedReader bReader = null;
				String line;
		...
		public void reduce(IntWritable key, Iterable<PairWritable> ps, Context context)
			...
			try {
				try {
					...
					score_div_leaf = Xgb1.gain_n(data, gamma, lambda, alpha, eta);
				} catch (Exception e) {
					e.printStackTrace();
				}
	...
	private final static Pair<Double, Pair<Double, Double>> gain_n(List<Pair<Double, Pair<Double, Double>>> data,
		...
		List<Pair<Double, Double>> GH = Xgb1.GH_square(cat_pred);
		double G = 0.0, H = 0.0, Gl = 0.0, Hl = 0.0, Gr = 0.0, Hr = 0.0;
		double score = -Double.MAX_VALUE;
		double new_OBJ = 0.0;
		double div = 0.0;
		double leaf = Xgb1.round(eta * res_sum / (data_size + 1), 8);
		...
		for (int i = 0; i < data_size - 1; i++) {
			...
			if (Double.compare(data.get(i).getL(), data.get(i + 1).getL()) != 0) {
				new_OBJ = Xgb1.round((double) (Math.pow(Gl + alpha, 2) / (Hl + lambda)
						+ Math.pow(Gr + alpha, 2) / (Hr + lambda) - OBJ - gamma), 6);
				if (new_OBJ > score) {
					score = new_OBJ;
					div = Xgb1.round((data.get(i).getL() + data.get(i + 1).getL()) / 2, 4);
				}
			}
		}
