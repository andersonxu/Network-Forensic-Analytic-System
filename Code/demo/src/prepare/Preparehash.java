...
public class Preparehash {
	...
	private final static void prepare(String path) throws FileNotFoundException, UnsupportedEncodingException {
		PrintWriter writer = new PrintWriter(path + "hash.txt", "UTF-8");
		...
		try {
			...
			while ((line = bReader.readLine()) != null) {
				...
				statistics.put(ans, (Math.abs(ans.hashCode()) % 4095));
			}
			for (Entry<String, Integer> entry : statistics.entrySet()) {
				writer.println("hash.put(" + entry.getValue() + ",\"" + entry.getKey() + "\");");
			}
